...
docker run --rm \
  ...
  -v "${SRC_ROOT}/user_data/freqaimodels:/src/freqaimodels:ro" \
  --entrypoint bash \
  --user root \
  "${IMAGE}" \
  -c '
    set -euo pipefail
    mkdir -p /data/strategies /data/freqaimodels /data/models /data/data /data/logs /data/backtest_results
    cp /src/config.json /data/config.json
    cp /src/RebelLiquidation.py /data/strategies/RebelLiquidation.py
    cp /src/freqaimodels/RebelXGBoostClassifier.py /data/freqaimodels/RebelXGBoostClassifier.py
    touch /data/strategies/__init__.py
    touch /data/freqaimodels/__init__.py
    python3 -m json.tool /data/config.json >/dev/null
    python3 -m py_compile /data/strategies/RebelLiquidation.py
    python3 -m py_compile /data/freqaimodels/RebelXGBoostClassifier.py
