"""
MomentumBG15_v3 — Trend Confirmation Filter

Based on v2 with one critical addition:

CHANGE 4: EMA200 Trend Confirmation
  - LONG only when close > EMA200 (price above long-term trend)
  - SHORT only when close < EMA200 (price below long-term trend)
  - This filters out ~65% of bad entries that went against the trend
  - v2 had 35.1% WR because entries were noise without trend context

All v2 changes preserved:
  - RSI thresholds: 50/50 (widened from 42/58)
  - No regime filter in entry
  - MACD rising/falling (not absolute level)
  
Everything else UNTOUCHED from v2:
  - Stoploss: -1.8% (static)
...
class MomentumBG15_v3(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        try:
            ...
            # CHANGE 4: EMA200 trend confirmation
            # LONG: close > EMA200 (uptrend confirmation)
            # SHORT: close < EMA200 (downtrend confirmation)
            # Plus v2 conditions: RSI filter + MACD direction
