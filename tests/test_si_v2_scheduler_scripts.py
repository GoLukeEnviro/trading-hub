...
SCRIPTS = {
    ...
    "t4_watcher_job": ROOT / "orchestrator/scripts/si_v2_t4_watcher_job.sh",
}
...
def test_t4_watcher_job_wrapper_is_detection_only() -> None:
    watcher_job = _read("t4_watcher_job")
    assert "/opt/data/logs/si-v2-t4-watcher" in watcher_job
    assert "SI_V2_T4_WATCHER_ALERT=MEASUREMENT_READY" in watcher_job
    assert "run_measurement_decision_engine_read_only" not in watcher_job
    assert "execute_apply" not in watcher_job
    assert "docker compose up" not in watcher_job
