...
Default provenance is now FreqForge_Gate0_Core_v1 (not FreqForge_Override).
...
class StrategyProvenance:
    ...
    uses_fleet_risk_manager: bool = False  # Replaced with noop stubs for Gate-0
    uses_primo_signal: bool = False  # Replaced with always-open gate for Gate-0
    uses_exit_agent: bool = False
    uses_dynamic_risk_gates: bool = False  # Noop stubs for Gate-0
    ...
    re_ratification_note: str = (
        "C5.3 corrective: FreqForge_Gate0_Core_v1 is a stripped research variant. "
        "Primo signals replaced with always-open gate, FleetRiskManager with noop stubs, "
        "AI/Shadow/LLM paths removed entirely. Default provenance is FreqForge_Gate0_Core_v1. "
        "Luke must explicitly re-ratify before any holdout evaluation."
    )
