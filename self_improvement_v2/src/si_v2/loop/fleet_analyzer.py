...
def analyze_fleet(evidence_list: list[BotEvidence], cycle_id: str) -> FleetDecision:
    ...
    # Fleet verdict:
    #   GREEN: all 4 bots ping_ok, none have ping_failed, and all 4 produced a decision
    #   YELLOW: at least one bot missing env vars OR partial ping success OR partial auth failure
    #   RED:    no bots processed at all OR all ping failed OR all auth failed
    if total == 0:
        verdict = "RED"
    ...
    elif ping_failed > 0 or status_yellow_missing > 0 or status_failed > 0:
        verdict = "YELLOW"
        reason = (
            f"partial success: {ping_ok}/{total} ping_ok, "
            f"{ping_failed} ping_failed, "
