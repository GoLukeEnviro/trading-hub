...
| `docs/roadmap/canonical-program-roadmap.md` | Derived view | new | G0.1 (generated) |
...
| `orchestrator/scripts/render_canonical_roadmap.py` | Tooling | new | G0.2 |
...
CI runs the renderer and `git diff --exit-code` on the output; any drift fails
the `governance-consistency` job.
...
- generated `canonical-program-roadmap.md` (Derived View);
...
- `render_canonical_roadmap.py`;
- `governance_consistency_check.py`;
- `test_governance_consistency.py`;
- `governance-consistency` required job in `main-gate.yml`;
